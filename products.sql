...
select a.product_id,a.product_name,a.price,a.group_id,avg(a.price) over (partition by a.group_id) as average_group_price
from products a 
inner join product_groups b
on a.group_id=b.group_id 
order by avg(a.price) over(partition by a.group_id) desc , a.price desc ; 
...
select group_name as product_group,round(avg(price),2) as average_price 
from products
...
select product_name,cum_dist from 
	(select product_name,price,
		cume_dist() over (order by price)*100 as cum_dist
	from products) as x; 

-- percent_rank
select product_name,percent_rank_ from 
	(select product_name,price,
		percent_rank() over (order by price)*100 as percent_rank_
	from products) as x; 
